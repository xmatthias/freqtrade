...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Optional[Exchange] = None) -> None:
        ...
        if not exchange:
            exchange = self.loop.run_until_complete(
                ExchangeResolver.load_exchange(self.config, load_leverage_tiers=True))
        ...
        asyncio.get_event_loop().run_until_complete(migrate_data(config, self.exchange))
