...
class Okx(Exchange):
    ...
    async def additional_exchange_init(self) -> None:
        ...
        try:
            if self.trading_mode == TradingMode.FUTURES and not self._config["dry_run"]:
                accounts = await self._api_async.fetch_accounts()
    ...
    async def __fetch_leverage_already_set(self, pair: str, leverage: float, side: BuySell) -> bool:
        try:
            res_lev = await self._api_async.fetch_leverage(
                symbol=pair,
                params={
    ...
    async def _lev_prep(self, pair: str, leverage: float, side: BuySell, accept_fail: bool = False):
        if self.trading_mode != TradingMode.SPOT and self.margin_mode is not None:
            try:
                ...
                already_set = await self.__fetch_leverage_already_set(pair, leverage, side)
                if not already_set:
                    raise TemporaryError(
    ...
    async def fetch_stoploss_order(
        ...
        try:
            ...
            return await self._convert_stop_order(pair, order_id, order_reg)
        except ccxt.OrderNotFound:
            pass
        params2 = {"stop": True, "ordType": "conditional"}
        for method in (
            self._api_async.fetch_open_orders,
            self._api_async.fetch_closed_orders,
            ...
            try:
                ...
                if orders_f:
                    order = orders_f[0]
                    return await self._convert_stop_order(pair, order_id, order)
            except ccxt.BaseError:
                pass
