...
class Exchange:
    ...
    def __init__(self, config: Config, *, exchange_config: Optional[ExchangeConfig] = None) -> None:
        """
        ...
        self.__exchange_config = exchange_conf
    ...
    async def init_exchange(
        self, load_markets: bool = True, validate: bool = True, load_leverage_tiers: bool = False
    ) -> None:
        exchange_conf = self.__exchange_config
        ...
        self._api = await self._init_ccxt(exchange_conf, True, ccxt_config)
        ...
        self._api_async = await self._init_ccxt(exchange_conf, False, ccxt_async_config)
        ...
        if (
            ...
            self._ws_async = await self._init_ccxt(exchange_conf, False, ccxt_async_config)
        ...
        if validate:
            ...
            self.required_candle_call_count = self.validate_required_startup_candles(
                self._startup_candle_count, self._config.get("timeframe", "")
            )

        if self.trading_mode != TradingMode.SPOT and load_leverage_tiers:
            await self.fill_leverage_tiers()
        await self.additional_exchange_init()
    ...
    async def create_dry_run_order(
        self,
        pair: str,
        ordertype: str,
        side: str,
    ...
    async def get_dry_market_fill_price(
    ...
    async def _dry_is_price_crossed(
        self,
        pair: str,
        side: str,
        limit: float,
    ...
    async def _lev_prep(self, pair: str, leverage: float, side: BuySell, accept_fail: bool = False):
        if self.trading_mode != TradingMode.SPOT:
            await self.set_margin_mode(pair, self.margin_mode, accept_fail)
            await self._set_leverage(leverage, pair, accept_fail)
    ...
    async def create_stoploss(
        ...
        try:
            ...
            await self._lev_prep(pair, leverage, side, accept_fail=True)
            order = await self._api.create_order(
                symbol=pair,
                type=ordertype,
                side=side,
                amount=amount,
    ...
    async def fetch_order_emulated(self, order_id: str, pair: str, params: Dict) -> Dict:
        """
        Emulated fetch_order if the exchange doesn't support fetch_order, but requires separate
        calls for open and closed orders.
        """
        try:
            order = await self._api_async.fetch_open_order(order_id, pair, params=params)
            self._log_exchange_response("fetch_open_order", order)
            order = self._order_contracts_to_amount(order)
            return order
        except ccxt.OrderNotFound:
            try:
                order = await self._api_async.fetch_closed_order(order_id, pair, params=params)
    ...
    async def fetch_order(self, order_id: str, pair: str, params: Optional[Dict] = None) -> Dict:
        ...
        try:
            if not self.exchange_has("fetchOrder"):
                return await self.fetch_order_emulated(order_id, pair, params)
            order = await self._api.fetch_order(order_id, pair, params=params)
    ...
    @retrier_async
    async def cancel_order(self, order_id: str, pair: str, params: Optional[Dict] = None) -> Dict:
        if self._config["dry_run"]:
            try:
        ...
        if params is None:
            params = {}
        try:
            order = await self._api_async.cancel_order(order_id, pair, params=params)
    ...
    async def cancel_stoploss_order_with_result(
    ...
    async def fetch_positions(self, pair: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            if pair:
                symbols.append(pair)
            positions: List[Dict] = await self._api_async.fetch_positions(symbols)
    ...
    async def get_tickers(
        ...
        try:
            ...
            raise OperationalException(
                f"Exchange {self.name} does not support fetching tickers in batch. " f"Message: {e}"
            ) from e
    ...
    @retrier
    def fetch_ticker_sync(self, pair: str) -> Ticker:
        try:
    ...
    async def fetch_l2_order_book(self, pair: str, limit: int = 100) -> dict:
        ...
        try:
            ...
            raise OperationalException(
                f"Exchange {self.name} does not support fetching order book." f"Message: {e}"
            ) from e
        except ccxt.DDoSProtection as e:
            raise DDosProtection(e) from e
        except (ccxt.NetworkError, ccxt.ExchangeError) as e:
    ...
    @retrier
    def fetch_l2_order_book_sync(self, pair: str, limit: int = 100) -> OrderBook:
        """
    ...
    def get_rate_sync(self, pair: str, refresh: bool, side: EntryExit, is_short: bool) -> float:
        ...
        return asyncio.run_coroutine_threadsafe(
            self.get_rate(pair, refresh, side, is_short), self.loop
        ).result()
    ...
    async def get_rates(self, pair: str, refresh: bool, is_short: bool) -> Tuple[float, float]:
        ...
        if not entry_rate and entry_pricing.get("use_order_book", False):
            ...
            order_book = await self.fetch_l2_order_book(pair, order_book_top)
            entry_rate = await self.get_rate(
    ...
    def get_fee(
        ...
        try:
            ...
            return self._api_async.calculate_fee(
                symbol=symbol,
                type=order_type,
                side=side,
                amount=amount,
    ...
    async def extract_cost_curr_rate(
        self, fee: Dict, symbol: str, cost: float, amount: float
        ...
        return (
            float(fee["cost"]),
            fee["currency"],
            await self.calculate_fee_rate(fee, symbol, cost, amount),
        )
    ...
    async def get_historic_ohlcv(
        ...
        """
        pair, _, _, data, _ = await self._async_get_historic_ohlcv(
            pair=pair,
            timeframe=timeframe,
            since_ms=since_ms,
            until_ms=until_ms,
    ...
    async def get_historic_trades(
        ...
        return await self._async_get_trade_history(
            pair=pair, since=since, until=until, from_id=from_id
        )
    ...
    async def _get_funding_fees_from_exchange(
        ...
        try:
            funding_history = await self._api_async.fetch_funding_history(symbol=pair, since=since)
    ...
    async def _set_leverage(
        ...
        try:
            res = await self._api_async.set_leverage(symbol=pair, leverage=leverage)
    ...
    async def set_margin_mode(
        ...
        if params is None:
            params = {}
        try:
            res = await self._api_async.set_margin_mode(margin_mode.value, pair, params)
