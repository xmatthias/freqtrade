...
class Exchange:
    ...
    async def _dry_is_price_crossed(
            self, pair: str, side: str, limit: float, orderbook: Optional[OrderBook] = None,
    ...
    async def fetch_bids_asks(
            self, symbols: Optional[List[str]] = None, cached: bool = False) -> Dict:
        """
    ...
    async def get_rates(self, pair: str, refresh: bool, is_short: bool) -> Tuple[float, float]:
        ...
        if not entry_rate and entry_pricing.get('use_order_book', False):
            ...
            order_book = await self.fetch_l2_order_book(pair, order_book_top)
            entry_rate = await self.get_rate(pair, refresh, 'entry', is_short,
    ...
    @retrier_async
    async def set_margin_mode(self, pair: str, margin_mode: MarginMode, accept_fail: bool = False,
