...
async def _download_pair_history(
    pair: str,
    *,
    datadir: Path,
    exchange: Exchange,
...
async def refresh_backtest_ohlcv_data(
    exchange: Exchange,
    pairs: List[str],
    timeframes: List[str],
    datadir: Path,
...
async def _download_trades_history(
    exchange: Exchange,
    pair: str,
    *,
    new_pairs_days: int = 30,
    ...
    try:
        ...
        # Default since_ms to 30 days if nothing is given
        new_trades = await exchange.get_historic_trades(
            pair=pair,
            since=since,
            until=until,
            from_id=from_id,
...
async def download_data_main(config: Config) -> None:
    timerange = TimeRange()
    if "days" in config:
    ...
    try:
        if config.get("download_trades"):
            ...
            await migrate_data(config, exchange)
