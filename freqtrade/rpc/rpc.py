...
from typing import Any, Coroutine, Dict, Generator, List, Optional, Sequence, Tuple, Union
...
class RPC:
    ...
    def _run_async(self, coro: Coroutine, require_lock=False):
        ...
        if require_lock:
            ...
            resp = asyncio.run_coroutine_threadsafe(coro, self._freqtrade.loop).result()
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total_bot = 0.0
        try:
            tickers: Tickers = self._run_async(self._freqtrade.exchange.get_tickers(cached=True))
    ...
    async def __rpc_reload_trade_from_exchange(self, trade_id: int) -> Dict[str, str]:
    ...
    async def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = await self._freqtrade.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = await self._freqtrade.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                await self._freqtrade.handle_cancel_exit(
                    trade, order, oo, CANCEL_REASON["FORCE_EXIT"]
                )
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            await self._freqtrade.execute_trade_exit(
    ...
    def _rpc_force_exit(
        ...
        return self._run_async(
            self.__rpc_force_exit_async(trade_id, ordertype, amount), require_lock=True
        )
    ...
    async def __rpc_force_entry(
        ...
        *,
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = await self._freqtrade.wallets.get_trade_stake_amount(
        ...
        if await self._freqtrade.execute_entry(
            pair,
            stake_amount,
            price,
    ...
    async def __rpc_cancel_open_order(self, trade_id: int):
        ...
        for open_order in trade.open_orders:
            try:
                order = await self._freqtrade.exchange.fetch_order(open_order.order_id, trade.pair)
    ...
    async def __rpc_delete_async(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        if (
            ...
            for oslo in trade.open_sl_orders:
                try:
                    await self._freqtrade.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                    c_count += 1
                except ExchangeError:
                    pass
    ...
    def _rpc_analysed_history_full(
        ...
        strategy.dp = DataProvider(
            config, exchange=exchange, pairlists=None, loop=asyncio.new_event_loop()
        )
