...
async def forceentry(payload: ForceEnterPayload, rpc: RPC = Depends(get_rpc)):
...
async def forcesell(payload: ForceExitPayload, rpc: RPC = Depends(get_rpc)):
    ordertype = payload.ordertype.value if payload.ordertype else None
    return await rpc._rpc_forceexit(payload.tradeid, ordertype)
