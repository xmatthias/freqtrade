        ...
        for order in orders:
            try:
                ...
                if order.order_date_utc - timedelta(days=5) < datetime.now(timezone.utc):
                    ...
                    fo['status'] = 'canceled'
                    await self.handle_cancel_order(fo, order.trade,
...
#
    ...
    async def _notify_enter(self, trade: Trade, order: Order, order_type: Optional[str] = None,
...
#
    ...
    async def _check_and_execute_exit(self, trade: Trade, exit_rate: float,
        ...
        for should_exit in exits:
            if should_exit.exit_flag:
                ...
                exited = await self.execute_trade_exit(trade, exit_rate, should_exit,
                                                       exit_tag=exit_tag1)
                if exited:
                    return True
        return False
    ...
    async def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        try:
            ...
            await self.execute_trade_exit(trade, stop_price, exit_check=ExitCheckTuple(
    ...
    async def handle_stoploss_on_exchange(self, trade: Trade) -> bool:
        ...
        if (trade.is_open
            ...
            if await self.create_stoploss_order(trade=trade,
                                                stop_price=trade.stoploss_or_liquidation):
                return False
            else:
    ...
    async def handle_timedout(self, trade: Trade) -> None:
        ...
        if not_closed:
            if fully_cancelled or (order_obj and self.strategy.ft_check_timed_out(
                ...
                await self.handle_cancel_order(order, trade, constants.CANCEL_REASON['TIMEOUT'])
            else:
                await self.replace_order(order, order_obj, trade)

    async def handle_cancel_order(self, order: Dict, trade: Trade, reason: str) -> None:
        """
        ...
        if order['side'] == trade.entry_side:
            await self.handle_cancel_enter(trade, order, reason)
        else:
            canceled = await self.handle_cancel_exit(trade, order, reason)
    ...
    async def replace_order(self, order: Dict, order_obj: Optional[Order], trade: Trade) -> None:
        ...
        if order_obj and latest_candle_close_date > order_obj.order_date_utc:
            ...
            if order_obj.price != adjusted_entry_price:
                ...
                await self.handle_cancel_enter(trade, order, cancel_reason,
                                               replacing=replacing)
        ...
        for trade in Trade.get_open_order_trades():
            ...
            if order['side'] == trade.entry_side:
                await self.handle_cancel_enter(
                    trade, order, constants.CANCEL_REASON['ALL_CANCELLED'])
    ...
    async def handle_cancel_enter(
        ...
        await self.wallets.update()
        await self._notify_enter_cancel(trade, order_type=self.strategy.order_types['entry'],
                                        reason=reason)
        return was_trade_fully_canceled

    async def handle_cancel_exit(self, trade: Trade, order: Dict, reason: str) -> bool:
        """
        ...
        if order['status'] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            try:
                co = await self.exchange.cancel_order_with_result(
                    order['id'], trade.pair, trade.amount)
            except InvalidOrderException:
                logger.exception(
            ...
            await self.update_trade_state(trade, trade.open_order_id, co)
...
#
    ...
    async def order_close_notify(
        ...
        if order.ft_order_side == trade.exit_side:
            ...
            if send_msg and not stoploss_order and not trade.open_order_id:
                await self._notify_exit(trade, '', fill=True, sub_trade=sub_trade, order=order)
            if not trade.is_open:
                self.handle_protections(trade.pair, trade.trade_direction)
        elif send_msg and not trade.open_order_id and not stoploss_order:
            # Enter fill
            await self._notify_enter(trade, order, fill=True, sub_trade=sub_trade)
    ...
    async def apply_fee_conditional(
            self, trade: Trade, trade_base_currency: str, amount: float, fee_abs: float,
            order_obj: Order) -> Optional[float]:
        """
            ...
            if fee_rate is None or fee_rate < 0.02:
                ...
                if trade_base_currency == fee_currency:
                    ...
                    return await self.apply_fee_conditional(
                        trade, trade_base_currency, amount=order_amount, fee_abs=fee_cost,
                        order_obj=order_obj)
                return None
