        ...
        for order in orders:
            try:
                ...
                if order.order_date_utc - timedelta(days=5) < datetime.now(timezone.utc):
                    ...
                    fo['status'] = 'canceled'
                    await self.handle_timedout_order(fo, order.trade)
...
#
    ...
    async def create_stoploss_order(self, trade: Trade, stop_price: float) -> bool:
        ...
        """
        try:
            stoploss_order = await self.exchange.stoploss(
                pair=trade.pair,
                amount=trade.amount,
    ...
    async def handle_timedout(self, trade: Trade) -> None:
        ...
        if not_closed:
            if fully_cancelled or (order_obj and self.strategy.ft_check_timed_out(
                ...
                await self.handle_timedout_order(order, trade)
            else:
                await self.replace_order(order, order_obj, trade)

    async def handle_timedout_order(self, order: Dict, trade: Trade) -> None:
        """
        ...
        if order['side'] == trade.entry_side:
            ...
            canceled = await self.handle_cancel_exit(
    ...
    async def handle_cancel_exit(self, trade: Trade, order: Dict, reason: str) -> bool:
        ...
        if order['status'] not in constants.NON_OPEN_EXCHANGE_STATES:
            ...
            try:
                co = await self.exchange.cancel_order_with_result(trade.open_order_id, trade.pair,
                                                                  trade.amount)
            except InvalidOrderException:
                logger.exception(
    ...
    async def execute_trade_exit(
        ...
        amount = await self._safe_exit_amount(trade.pair, sub_trade_amt or trade.amount)
