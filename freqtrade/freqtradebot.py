...
#
    ...
    async def check_and_call_adjust_trade_position(self, trade: Trade):
        ...
        current_rate = await self.exchange.get_rate(trade.pair, refresh=True, side="buy")
    ...
    async def execute_entry(self, pair: str, stake_amount: float, price: Optional[float] = None, *,
        ...
        enter_limit_requested, stake_amount = await self.get_valid_enter_price_and_stake(
        ...
        if not pos_adjust and not strategy_safe_wrapper(
            ...
            return False
        order = await self.exchange.create_order(pair=pair, ordertype=order_type, side="buy",
...
#
    ...
    async def handle_trade(self, trade: Trade) -> bool:
        ...
        sell_rate = await self.exchange.get_rate(trade.pair, refresh=True, side="sell")
        if await self._check_and_execute_exit(trade, sell_rate, buy, sell, exit_tag):
            return True
    ...
    async def _check_and_execute_exit(self, trade: Trade, exit_rate: float,
        ...
        if should_sell.sell_flag:
            ...
            await self.execute_trade_exit(trade, exit_rate, should_sell, exit_tag=exit_tag)
            return True
        return False
    ...
    async def execute_trade_exit(
        ...
        try:
            ...
            order = await self.exchange.create_order(pair=trade.pair,
