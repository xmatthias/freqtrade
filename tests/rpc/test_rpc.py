...
async def test_rpc_trade_history(mocker, default_conf, markets, fee, is_short):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
@pytest.mark.parametrize('is_short', [True, False])
async def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
...
async def test_rpc_trade_statistics(default_conf, ticker, ticker_sell_up, fee,
...
async def test_rpc_trade_statistics_closed(mocker, default_conf, ticker, fee,
...
async def test_rpc_start(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
...
async def test_rpc_stop(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        fetch_ticker=MagicMock()
...
async def test_rpc_stopbuy(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        fetch_ticker=MagicMock()
...
async def test_rpc_forceexit(default_conf, ticker, fee, mocker) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    ...
    with pytest.raises(RPCException, match=r'.*trader is not running*'):
        await rpc._rpc_forceexit(None)

    freqtradebot.state = State.RUNNING
    with pytest.raises(RPCException, match=r'.*invalid argument*'):
        await rpc._rpc_forceexit(None)

    msg = await rpc._rpc_forceexit('all')
    ...
    await freqtradebot.enter_positions()
    msg = await rpc._rpc_forceexit('2')
    ...
    # check that the trade is called, which is done by ensuring exchange.cancel_order is called
    msg = await rpc._rpc_forceexit('4')
    ...
    msg = await rpc._rpc_forceexit('3')
    assert msg == {'result': 'Created sell order for trade 3.'}
    # status quo, no exchange calls
    assert cancel_order_mock.call_count == 3


async def test_performance_handle(default_conf, ticker, limit_buy_order, fee,
...
async def test_enter_tag_performance_handle(default_conf, ticker, limit_buy_order, fee,
                                            limit_sell_order, mocker) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
...
async def test_enter_tag_performance_handle_2(mocker, default_conf, markets, fee):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
async def test_sell_reason_performance_handle(default_conf, ticker, limit_buy_order, fee,
                                              limit_sell_order, mocker) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
...
async def test_sell_reason_performance_handle_2(mocker, default_conf, markets, fee):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
async def test_mix_tag_performance_handle(default_conf, ticker, limit_buy_order, fee,
                                          limit_sell_order, mocker) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
...
async def test_mix_tag_performance_handle_2(mocker, default_conf, markets, fee):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
async def test_rpc_count(mocker, default_conf, ticker, fee) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        get_balances=MagicMock(return_value=ticker),
...
async def test_rpc_forceentry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
...
async def test_rpc_forceentry_stopped(mocker, default_conf) -> None:
    default_conf['forcebuy_enable'] = True
...
async def test_rpc_forceentry_disabled(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
...
async def test_rpc_forceentry_wrong_mode(mocker, default_conf) -> None:
    default_conf['forcebuy_enable'] = True
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
...
async def test_rpc_delete_lock(mocker, default_conf):
    freqtradebot = await get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)
    pair = 'ETH/BTC'
...
async def test_rpc_whitelist(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
...
async def test_rpc_whitelist_dynamic(mocker, default_conf) -> None:
...
async def test_rpc_blacklist(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
...
async def test_rpc_edge_disabled(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    freqtradebot = await get_patched_freqtradebot(mocker, default_conf)
    rpc = RPC(freqtradebot)
...
async def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    ...
    freqtradebot = await get_patched_freqtradebot(mocker, edge_conf)
