...
async def test_rpc_trade_history(mocker, default_conf, markets, fee):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
async def test_buy_tag_performance_handle(default_conf, ticker, limit_buy_order, fee,
                                          limit_sell_order, mocker) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
...
async def test_buy_tag_performance_handle_2(mocker, default_conf, markets, fee):
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        markets=PropertyMock(return_value=markets)
...
async def test_rpcforcebuy(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    with pytest.raises(RPCException, match=r'position for ETH/BTC already open - id: 1'):
        await rpc._rpc_forcebuy(pair, 0.0001)
    pair = 'XRP/BTC'
    trade = await rpc._rpc_forcebuy(pair, 0.0001, order_type='limit')
    assert isinstance(trade, Trade)
    assert trade.pair == pair
    ...
    with pytest.raises(RPCException,
        ...
        await rpc._rpc_forcebuy('LTC/ETH', 0.0001)

    # Test with defined stake_amount
    pair = 'LTC/BTC'
    trade = await rpc._rpc_forcebuy(pair, 0.0001, order_type='limit', stake_amount=0.05)
...
async def test_rpcforcebuy_stopped(mocker, default_conf) -> None:
    default_conf['forcebuy_enable'] = True
    ...
    pair = 'ETH/BTC'
    with pytest.raises(RPCException, match=r'trader is not running'):
        await rpc._rpc_forcebuy(pair, None)


async def test_rpcforcebuy_disabled(mocker, default_conf) -> None:
    mocker.patch('freqtrade.rpc.telegram.Telegram', MagicMock())
