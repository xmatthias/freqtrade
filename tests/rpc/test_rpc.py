...
from tests.conftest import (
    ...
    get_patched_freqtradebot_thread,
    patch_get_signal,
)
...
async def test_rpc_balance_handle(default_conf_usdt, mocker, tickers):
    mock_balance = {
        "BTC": {
            "free": 0.01,
            "total": 0.012,
...
async def test_rpc_stopentry(mocker, default_conf) -> None:
    mocker.patch("freqtrade.rpc.telegram.Telegram", MagicMock())
    mocker.patch.multiple(EXMS, fetch_ticker=MagicMock())
...
async def test_rpc_add_and_delete_lock(mocker, default_conf):
