...
async def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch('freqtrade.exchange.Exchange.get_trades_for_order', return_value=[])
    mocker.patch('freqtrade.freqtradebot.FreqtradeBot.get_real_amount',
                 get_mock_coro(limit_order[entry_side(is_short)]['amount']))
...
async def test_get_real_amount_quote(
    ...
    assert await freqtrade.get_real_amount(
        trade, buy_order_fee, order_obj) == amount - (amount * 0.001)
    assert log_has(
...
async def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    assert await freqtrade.get_real_amount(trade, buy_order_fee, order_obj) == amount
...
async def test_get_real_amount(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    assert await freqtrade.get_real_amount(
        trade, buy_order, order_obj) == amount - fee_reduction_amount
...
async def test_get_real_amount_invalid_order(
    ...
    assert await freqtrade.get_real_amount(trade, limit_buy_order_usdt, order_obj) == amount
...
async def test_get_real_amount_fees_order(
    ...
    assert await freqtrade.get_real_amount(
        trade, market_buy_order_usdt_doublefee, order_obj) == 30.0
...
async def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert await freqtrade.get_real_amount(trade, order, order_obj) == amount
