...
async def test_create_trade_minimal_amount(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    enter_mock = get_mock_coro(return_value=limit_order_open[enter_side(is_short)])
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        fetch_ticker=ticker_usdt,
...
async def test_process_trade_creation(default_conf_usdt, ticker_usdt, limit_order, limit_order_open,
    ...
    mocker.patch.multiple(
        'freqtrade.exchange.Exchange',
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value=limit_order_open[enter_side(is_short)]),
        fetch_order=get_mock_coro(return_value=limit_order[enter_side(is_short)]),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
...
async def test_execute_entry_confirm_error(
    ...
    mocker.patch.multiple(
        ...
        create_order=get_mock_coro(return_value=limit_order[enter_side(is_short)]),
...
async def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch('freqtrade.exchange.Exchange.fetch_order',
                 get_mock_coro(limit_order[enter_side(is_short)]))
    mocker.patch('freqtrade.exchange.Exchange.get_trades_for_order', return_value=[])
    mocker.patch('freqtrade.freqtradebot.FreqtradeBot.get_real_amount',
                 get_mock_coro(limit_order[enter_side(is_short)]['amount']))
...
async def test_update_trade_state_exception(
        mocker, default_conf_usdt, is_short, limit_order, caplog) -> None:
    order = limit_order[enter_side(is_short)]
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mocker.patch('freqtrade.exchange.Exchange.fetch_order', return_value=order)
...
async def test_handle_cancel_enter(
    ...
    assert not await freqtrade.handle_cancel_enter(trade, limit_order[enter_side(is_short)], reason)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_ignore_roi_if_buy_signal(default_conf_usdt, limit_order, limit_order_open, is_short,
                                        fee, mocker) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    eside = enter_side(is_short)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_disable_ignore_roi_if_buy_signal(default_conf_usdt, limit_order, limit_order_open,
