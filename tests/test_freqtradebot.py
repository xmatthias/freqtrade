...
async def test_enter_positions_no_pairs_in_whitelist(default_conf, ticker, limit_buy_order, fee,
                                                     mocker, caplog) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch.multiple(
...
async def test_enter_positions(mocker, default_conf, caplog) -> None:
...
async def test_enter_positions_exception(mocker, default_conf, caplog) -> None:
    freqtrade = get_patched_freqtradebot(mocker, default_conf)
...
async def test_update_trade_state_withorderdict(default_conf, trades_for_order, limit_buy_order,
                                                fee, mocker):
...
async def test_update_trade_state_withorderdict_rounding_fee(default_conf, trades_for_order, fee,
...
async def test_get_real_amount_stake(default_conf, trades_for_order, buy_order_fee, fee, mocker):
...
async def test_get_real_amount_no_currency_in_fee(default_conf, trades_for_order, buy_order_fee,
                                                  fee, mocker):
...
async def test_get_real_amount_BNB(default_conf, trades_for_order, buy_order_fee, fee, mocker):
...
async def test_get_real_amount_multi(default_conf, trades_for_order2, buy_order_fee, caplog,
                                     fee, mocker):
...
async def test_get_real_amount_fromorder(default_conf, trades_for_order, buy_order_fee, fee,
    ...
    # Amount is reduced by "fee"
    assert await freqtrade.get_real_amount(trade, limit_buy_order) == amount - 0.004
    assert log_has('Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
                   'open_rate=0.24544100, open_since=closed) (from 8.0 to 7.996).',
                   caplog)
...
async def test_get_real_amount_invalid(default_conf, trades_for_order, buy_order_fee, fee, mocker):
...
async def test_order_book_depth_of_market(default_conf, ticker, limit_buy_order_open,
                                          limit_buy_order, fee, mocker, order_book_l2):
...
async def test_order_book_depth_of_market_high_delta(default_conf, ticker, limit_buy_order,
