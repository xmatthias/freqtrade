...
async def test_freqtradebot_state(mocker, default_conf_usdt, markets) -> None:
...
async def test_process_stopped(mocker, default_conf_usdt) -> None:
...
async def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    await freqtrade.process()
    assert freqtrade.rpc.process_msg_queue.call_count == 1


async def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
...
async def test_get_trade_stake_amount(default_conf_usdt, ticker, mocker) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    ...
    result = await freqtrade.wallets.get_trade_stake_amount('ETH/USDT', 1)
...
async def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            with pytest.raises(DependencyException):
                await freqtrade.wallets.get_trade_stake_amount('ETH/USDT', 1)


async def test_edge_called_in_process(mocker, edge_conf) -> None:
    patch_RPCManager(mocker)
    patch_edge(mocker)
...
async def test_edge_overrides_stoploss(limit_order, fee, caplog, mocker,
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=get_mock_coro(return_value=ticker_val),
        get_fee=fee,
    )
    #############################################
...
async def test_total_open_trades_stakes(mocker, default_conf_usdt, ticker_usdt, fee) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
...
async def test_create_trade(default_conf_usdt, ticker_usdt, limit_order,
    ...
    await freqtrade.create_trade('ETH/USDT')
...
async def test_create_trade_no_stake_amount(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
...
async def test_create_trade_minimal_amount(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    enter_mock = get_mock_coro(return_value=limit_order_open[entry_side(is_short)])
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    if create:
        assert await freqtrade.create_trade('ETH/USDT')
        if amount_enough:
        ...
        assert not await freqtrade.create_trade('ETH/USDT')
...
async def test_enter_positions_no_pairs_left(
...
async def test_enter_positions_global_pairlock(
        default_conf_usdt, ticker_usdt, limit_buy_order_usdt, fee, mocker, caplog) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value={'id': limit_buy_order_usdt['id']}),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
...
@pytest.mark.parametrize('is_short', [False, True])
async def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
...
async def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
...
async def test_create_trades_multiple_trades(
...
@pytest.mark.parametrize('is_short', [False, True])
async def test_process_trade_creation(default_conf_usdt, ticker_usdt, limit_order, limit_order_open,
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value=limit_order_open[entry_side(is_short)]),
        fetch_order=get_mock_coro(return_value=limit_order[entry_side(is_short)]),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
    ...
    assert pytest.approx(trade.amount) == 60 / (await ticker_usdt())[ticker_side]
...
async def test_process_trade_handling(
        default_conf_usdt, ticker_usdt, limit_buy_order_usdt_open, fee, mocker) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value=limit_buy_order_usdt_open),
        fetch_order=get_mock_coro(return_value=limit_buy_order_usdt_open),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
...
async def test_process_trade_no_whitelist_pair(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    mocker.patch.multiple(
        ...
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value={'id': limit_buy_order_usdt['id']}),
        fetch_order=get_mock_coro(return_value=limit_buy_order_usdt),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
...
async def test_execute_entry(mocker, default_conf_usdt, fee, limit_order,
    ...
    mocker.patch('freqtrade.wallets.Wallets.get_available_stake_amount', return_value=1000)
    pair = 'ETH/USDT'

    assert not await freqtrade.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    open_order['id'] = '33'
    fix_price = 0.06
    assert await freqtrade.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=order))
    ...
    # Fail to get price...
    mocker.patch(f'{EXMS}.get_rate', get_mock_coro(return_value=0.0))

    with pytest.raises(PricingError, match="Could not determine entry price."):
        await freqtrade.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f'{EXMS}.get_rate', get_mock_coro(0.50))
    order['status'] = 'open'
    order['id'] = '5566'
    ...
    assert await freqtrade.execute_entry(pair, 200, leverage_=3)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_entry_confirm_error(
        mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mocker.patch.multiple(
        EXMS,
        ...
        create_order=get_mock_coro(return_value=limit_order[entry_side(is_short)]),
        get_rate=get_mock_coro(return_value=0.11),
        get_min_pair_stake_amount=MagicMock(return_value=1),
        get_fee=fee,
    )
    mocker.patch('freqtrade.wallets.Wallets.get_available_stake_amount', return_value=50)
    stake_amount = 2
    pair = 'ETH/USDT'
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_entry_min_leverage(
        mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_stoploss_on_exchange(mocker, default_conf_usdt, fee, caplog, is_short,
                                           limit_order) -> None:
    ...
    hanging_stoploss_order = get_mock_coro(return_value={'id': '13434334', 'status': 'open'})
    ...
    canceled_stoploss_order = get_mock_coro(return_value={'id': '13434334', 'status': 'canceled'})
    ...
    assert await freqtrade.handle_stoploss_on_exchange(trade) is True
...
async def test_handle_stoploss_on_exchange_emergency(mocker, default_conf_usdt, fee, is_short,
    ...
    stoploss_order_cancelled = get_mock_coro(side_effect=[{
        'id': "107",
        'status': 'canceled',
        'type': 'stop_loss_limit',
        'price': 3,
    ...
    stoploss = get_mock_coro(side_effect=InvalidOrderException())
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_stoploss_on_exchange_partial(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    stoploss_order_cancel = get_mock_coro(return_value={
        'id': "101",
        'status': 'canceled',
        'type': 'stop_loss_limit',
        'price': 3,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_sle_cancel_cant_recreate(mocker, default_conf_usdt, fee, caplog, is_short,
                                               limit_order) -> None:
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    await freqtrade.create_stoploss_order(trade, 200)
    assert trade.stoploss_order_id is None
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_create_stoploss_order_insufficient_funds(
...
@pytest.mark.usefixtures("init_persistence")
async def test_handle_stoploss_on_exchange_trailing(
    ...
    enter_order = limit_order[entry_side(is_short)]
    exit_order = limit_order[exit_side(is_short)]
    stoploss = get_mock_coro(return_value={'id': '13434334', 'status': 'open'})
    start_dt = dt_now()
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_stoploss_on_exchange_trailing_error(
    ...
    caplog.clear()
    cancel_mock = mocker.patch(f'{EXMS}.cancel_stoploss_order', get_mock_coro())
    mocker.patch(f'{EXMS}.create_stoploss', side_effect=ExchangeError())
    await freqtrade.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
async def test_stoploss_on_exchange_price_rounding(
    ...
    await freqtrade.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
@pytest.mark.parametrize("is_short", [False, True])
@pytest.mark.usefixtures("init_persistence")
async def test_handle_stoploss_on_exchange_custom_stop(
...
async def test_enter_positions(mocker, default_conf_usdt, return_value, side_effect,
                               log_message, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)

    mock_ct = get_mock_coro(
        return_value=return_value,
        side_effect=[
            DependencyException() for _ in
            range(len(default_conf_usdt['exchange']['pair_whitelist']))]
        if side_effect else None,
    )
    mocker.patch(
        'freqtrade.freqtradebot.FreqtradeBot.create_trade', mock_ct
    )
    n = await freqtrade.enter_positions()
    assert n == 0
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("is_short", [False, True])
async def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)

    mocker.patch('freqtrade.freqtradebot.FreqtradeBot.handle_trade', get_mock_coro(True))
    mocker.patch(f'{EXMS}.fetch_order',
                 get_mock_coro(limit_order[entry_side(is_short)]))
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=[])
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("is_short", [False, True])
async def test_exit_positions_exception(
        mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    order = limit_order[entry_side(is_short)]
    mocker.patch(f'{EXMS}.fetch_order', return_value=order)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    order = limit_order[entry_side(is_short)]
    ...
    assert not await freqtrade.update_trade_state(trade, None)
    assert log_has_re(r'Orderid for trade .* is empty.', caplog)
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once written to database
    await freqtrade.update_trade_state(trade, order_id)
    # Test amount not modified by fee-logic
    ...
    res = await freqtrade.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
...
async def test_update_trade_state_withorderdict(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_update_trade_state_exception(mocker, default_conf_usdt, is_short, limit_order,
                                            caplog) -> None:
    order = limit_order[entry_side(is_short)]
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mocker.patch(f'{EXMS}.fetch_order', return_value=order)
    ...
    await freqtrade.update_trade_state(trade, open_order_id)
    assert log_has('Could not update trade amount: ', caplog)


async def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_update_trade_state_sell(
    ...
    await freqtrade.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_trade_roi(default_conf_usdt, ticker_usdt, limit_order_open, fee, mocker,
                                caplog, is_short) -> None:
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_trade_use_exit_signal(
...
async def test_close_trade(
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        create_order=get_mock_coro(return_value=open_order),
        get_fee=fee,
    )
    freqtrade = FreqtradeBot(default_conf_usdt)
    await freqtrade.init_bot()
...
async def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_entry_usercustom(
    ...
    rpc_mock = patch_RPCManager(mocker)
    cancel_order_mock = get_mock_coro(return_value=old_order)
    cancel_enter_order = deepcopy(old_order)
    cancel_enter_order['status'] = 'canceled'
    cancel_order_wr_mock = get_mock_coro(return_value=cancel_enter_order)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_entry(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    limit_sell_order_old, fee, mocker, is_short
) -> None:
    old_order = limit_sell_order_old if is_short else limit_buy_order_old
    ...
    cancel_order_mock = get_mock_coro(return_value=limit_entry_cancel)
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_adjust_entry_cancel(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_adjust_entry_replace_fail(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    limit_sell_order_old, fee, mocker, caplog, is_short
) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    ...
    cancel_order_mock = get_mock_coro(return_value=limit_entry_cancel)
    fetch_order_mock = get_mock_coro(return_value=old_order)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_adjust_entry_replace_fail_create_order(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    limit_sell_order_old, fee, mocker, caplog, is_short
) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_adjust_entry_maintain_replace(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    limit_sell_order_old, fee, mocker, caplog, is_short
) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    ...
    freqtrade.get_valid_enter_price_and_stake = get_mock_coro(return_value={100, 10, 1})
    freqtrade.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_check_handle_cancelled_buy(
    default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    limit_sell_order_old, fee, mocker, caplog, is_short
) -> None:
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_buy_exception(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_exit_usercustom(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_exit(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_check_handle_cancelled_exit(
...
async def test_manage_open_orders_partial(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_partial_fee(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_manage_open_orders_partial_except(
    default_conf_usdt, ticker_usdt, open_trade, caplog, fee, is_short,
    limit_buy_order_old_partial, trades_for_order,
    limit_buy_order_old_partial_canceled, mocker
) -> None:
...
async def test_manage_open_orders_exception(default_conf_usdt, ticker_usdt, open_trade_usdt, mocker,
                                            caplog) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    cancel_order_mock = MagicMock()
    ...
    mocker.patch.multiple(
        ...
        handle_cancel_exit=get_mock_coro(None),
    )
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_cancel_enter(
        mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    ...
    cancel_order_mock = get_mock_coro(return_value=cancel_entry_order)
    ...
    assert await freqtrade.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order['filled'] = 0.01
    assert not await freqtrade.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not await freqtrade.handle_cancel_enter(
    ...
    mocker.patch('freqtrade.freqtradebot.asyncio.sleep')
    cbo['status'] = 'open'
    co_mock = mocker.patch(f'{EXMS}.cancel_order_with_result', get_mock_coro(cbo))
    fo_mock = mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(cbo))
    assert not await freqtrade.handle_cancel_enter(
...
async def test_handle_cancel_enter_exchanges(mocker, caplog, default_conf_usdt, is_short, fee,
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert await freqtrade.handle_cancel_enter(
...
@pytest.mark.usefixtures("init_persistence")
async def test_handle_cancel_enter_corder_empty(mocker, default_conf_usdt, limit_order, is_short,
                                                fee, cancelorder) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    ...
    mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(order))
    assert not await freqtrade.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1
...
async def test_handle_cancel_exit_limit(mocker, default_conf_usdt, fee, is_short,
    ...
    assert await freqtrade.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    ...
    assert not await freqtrade.handle_cancel_exit(trade, order, order_obj, reason)
...
async def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch(f'{EXMS}.get_min_pair_stake_amount', return_value=0.0)
...
async def test_execute_trade_exit_up(
        default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_up, mocker,
    ...
    price = (await ticker_usdt_sell_down()
             )['ask'] if is_short else (await ticker_usdt_sell_up())['bid']
    # Prevented sell ...
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_trade_exit_down(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_down,
...
async def test_execute_trade_exit_custom_exit_price(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_trade_exit_down_stoploss_on_exchange_dry_run(
...
async def test_execute_trade_exit_sloe_cancel_exception(
    ...
    await freqtrade.execute_trade_exit(trade=trade, limit=1234,
                                       exit_check=ExitCheckTuple(exit_type=ExitType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_trade_exit_with_stoploss_on_exchange(
    ...
    mocker.patch.multiple(
        ...
        _dry_is_price_crossed=get_mock_coro(side_effect=[True, False]),
    )
    ...
    await freqtrade.manage_open_orders()
    await freqtrade.exit_positions([trade])
...
async def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    mocker.patch.multiple(
        ...
        amount_to_precision=lambda s, x, y: y,
        price_to_precision=lambda s, x, y: y,
        _dry_is_price_crossed=get_mock_coro(side_effect=[False, True]),
    )
...
async def test_exit_profit_only(
...
async def test_sell_not_enough_balance(default_conf_usdt, limit_order, limit_order_open,
...
async def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    amount = 95.33
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_locked_pairs(default_conf_usdt, ticker_usdt, fee,
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open,
...
async def test_trailing_stop_loss(default_conf_usdt, limit_order_open,
...
async def test_trailing_stop_loss_positive(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_disable_ignore_roi_if_entry_signal(default_conf_usdt, limit_order, limit_order_open,
                                                  is_short, fee, mocker) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    eside = entry_side(is_short)
...
async def test_get_real_amount_quote(
    ...
    assert await freqtrade.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
async def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
                                          caplog, mocker):
    mocker.patch(f'{EXMS}.get_trades_for_order', return_value=trades_for_order)
    ...
    assert await freqtrade.get_real_amount(trade, buy_order_fee, order_obj) is None
...
async def test_get_real_amount(
    ...
    mocker.patch(f'{EXMS}.get_trades_for_order', get_mock_coro(trades_for_order))
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    if not use_ticker_usdt_rate:
        mocker.patch(f'{EXMS}.fetch_ticker', get_mock_coro(side_effect=ExchangeError()))

    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = await freqtrade.get_real_amount(trade, buy_order, order_obj)
...
async def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    assert await freqtrade.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            'Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
async def test_get_real_amount_invalid_order(
    ...
    assert await freqtrade.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
async def test_get_real_amount_fees_order(
    ...
    assert await freqtrade.get_real_amount(trade, market_buy_order_usdt_doublefee,
                                           order_obj) is None
...
async def test_get_real_amount_wrong_amount(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        await freqtrade.get_real_amount(trade, limit_buy_order_usdt, order_obj)


async def test_get_real_amount_wrong_amount_rounding(
        default_conf_usdt, trades_for_order, buy_order_fee, fee, mocker):
    ...
    assert pytest.approx(await freqtrade.get_real_amount(
        trade, limit_buy_order_usdt, order_obj)) == (amount * 0.001)


async def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    amount = 12345
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert await freqtrade.get_real_amount(trade, order, order_obj) is None


async def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    limit_buy_order_usdt = deepcopy(buy_order_fee)
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, 'LTC/ETH', 'buy')
    res = await freqtrade.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
async def test_apply_fee_conditional(default_conf_usdt, fee, mocker, caplog,
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert (await freqtrade.apply_fee_conditional(trade, 'LTC', amount,
                                                  fee_abs, order)) == amount_exp
    assert walletmock.call_count == 1
...
@pytest.mark.parametrize('is_short', [False, True])
async def test_order_book_depth_of_market(
...
async def test_order_book_entry_pricing1(mocker, default_conf_usdt, order_book_l2, exception_thrown,
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_l2_order_book=get_mock_coro(return_value=order_book) if order_book else order_book_l2,
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            await freqtrade.exchange.get_rate(
                'ETH/USDT', side="entry", is_short=False, refresh=True)
        assert log_has_re(
            r'ETH/USDT - Entry Price at location 1 from orderbook could not be determined.', caplog)
    else:
        assert await freqtrade.exchange.get_rate(
            'ETH/USDT', side="entry", is_short=False, refresh=True) == 0.043935
        assert ticker_usdt_mock.call_count == 0


async def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    """
    ...
    assert (await freqtrade._check_depth_of_market(
        'ETH/BTC', conf, side=SignalDirection.LONG)) is False


@pytest.mark.parametrize('is_short', [False, True])
async def test_order_book_exit_pricing(
    ...
    mocker.patch(f'{EXMS}.fetch_l2_order_book',
...
async def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
...
@pytest.mark.usefixtures("init_persistence")
async def test_sync_wallet_dry_run(
    ...
    bot = await get_patched_freqtradebot(mocker, default_conf_usdt)
    await bot.wallets.update()
    patch_get_signal(bot)
    assert bot.wallets.get_free('USDT') == 120.0

    n = await bot.enter_positions()
    assert n == 2
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == 2
...
async def test_cancel_all_open_orders(mocker, default_conf_usdt, fee, limit_order, limit_order_open,
    ...
    await freqtrade.cancel_all_open_orders()
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("is_short", [False, True])
async def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)

    await freqtrade.startup_update_open_orders()
...
@pytest.mark.usefixtures("init_persistence")
async def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("is_short", [False, True])
async def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    ...
    await freqtrade.handle_insufficient_funds(trades[3])
    ...
    await freqtrade.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    caplog.set_level(logging.DEBUG)
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mock_uts = mocker.patch('freqtrade.freqtradebot.FreqtradeBot.update_trade_state')
    ...
    await freqtrade.handle_insufficient_funds(trades[4])
...
@pytest.mark.parametrize('is_short', [True, False])
async def test_update_funding_fees(
    mocker,
    default_conf,
    time_machine,
    fee,
    ...
    await freqtrade.execute_entry('ETH/USDT', 123, is_short=is_short)
    await freqtrade.execute_entry('LTC/USDT', 2.0, is_short=is_short)
    await freqtrade.execute_entry('XRP/USDT', 123, is_short=is_short)
    multipl = 1 if is_short else -1
    trades = Trade.get_open_trades()
    assert len(trades) == 3
    for trade in trades:
        assert pytest.approx(trade.funding_fees) == 0
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(open_exit_order))
...
async def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_successful_buy_order))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=open_dca_order_1))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_dca_order_1))
    mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(return_value=closed_dca_order_1))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_dca_order_2))
    mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(return_value=closed_dca_order_2))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_sell_dca_order_1))
    mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(return_value=closed_sell_dca_order_1))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 get_mock_coro(return_value=closed_sell_dca_order_1))
    assert await freqtrade.execute_trade_exit(
        trade=trade, limit=8, exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
        sub_trade_amt=15)
...
async def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    """
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 get_mock_coro(return_value=closed_successful_buy_order))
    assert await freqtrade.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
    ...
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 get_mock_coro(return_value=closed_sell_dca_order_1))
    assert await freqtrade.execute_trade_exit(
        trade=trade, limit=ask, exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
    ...
    mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_sell_dca_order_2))
    mocker.patch(f'{EXMS}.fetch_order', get_mock_coro(return_value=closed_sell_dca_order_2))
    mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                 get_mock_coro(return_value=closed_sell_dca_order_2))
    assert await freqtrade.execute_trade_exit(
        trade=trade, limit=ask, exit_check=ExitCheckTuple(exit_type=ExitType.PARTIAL_EXIT),
        sub_trade_amt=amount)
    # Assert trade is as expected (averaged dca)
...
async def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    default_conf_usdt.update({
        "position_adjustment_enable": True,
        "stake_amount": 200.0,
        "dry_run_wallet": 1000.0,
    ...
    for idx, (order, result) in enumerate(data):
        amount = order[1]
        price = order[2]
        price_mock = get_mock_coro(return_value=price)
        mocker.patch.multiple(
            EXMS,
            get_rate=price_mock,
            fetch_ticker=get_mock_coro(return_value={
        ...
        mocker.patch(f'{EXMS}.create_order', get_mock_coro(return_value=closed_successful_order))
        mocker.patch(f'{EXMS}.fetch_order_or_stoploss_order',
                     get_mock_coro(return_value=closed_successful_order))
        if order[0] == 'buy':
            assert await freqtrade.execute_entry(pair, amount, trade=trade)
        else:
            assert await freqtrade.execute_trade_exit(
...
async def test_process_open_trade_positions_exception(
        mocker, default_conf_usdt, fee, caplog) -> None:
    default_conf_usdt.update({
        "position_adjustment_enable": True,
    })
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    ...
    await freqtrade.process_open_trade_positions()
    assert log_has_re(r"Unable to adjust position of trade for .*", caplog)


async def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    default_conf_usdt.update({
        "position_adjustment_enable": True,
        "max_entry_position_adjustment": 0,
    })
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    buy_rate_mock = get_mock_coro(return_value=10)
    mocker.patch.multiple(
        EXMS,
        get_rate=buy_rate_mock,
        fetch_ticker=get_mock_coro(return_value={
