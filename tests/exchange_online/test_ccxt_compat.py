...
class TestCCXTExchange:
    ...
    def test_ccxt_order_parse(self, exchange: EXCHANGE_FIXTURE_TYPE):
        ...
        if orders := EXCHANGES[exchange_name].get('sample_order'):
            pair = 'SOL/USDT'
            for order in orders:
                market = exch._api_async.markets[pair]
                po = exch._api_async.parse_order(order, market)
    ...
    async def test_ccxt_fetch_tickers(self, exchange: EXCHANGE_FIXTURE_TYPE):
        exch, exchangename = exchange
        pair = EXCHANGES[exchangename]['pair']

        tickers = await exch.get_tickers()
    ...
    async def test_ccxt_fetch_tickers_futures(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
    ...
    def test_ccxt_fetch_ticker_sync(self, exchange: EXCHANGE_FIXTURE_TYPE):
        exch, exchangename = exchange
        pair = EXCHANGES[exchangename]['pair']
    ...
    @pytest.mark.asyncio
    async def test_ccxt_fetch_ticker(self, exchange: EXCHANGE_FIXTURE_TYPE):
    ...
    def test_ccxt_fetch_l2_orderbook_sync(self, exchange: EXCHANGE_FIXTURE_TYPE):
        exch, exchangename = exchange
        pair = EXCHANGES[exchangename]['pair']
    ...
    @pytest.mark.asyncio
    async def test_ccxt_fetch_l2_orderbook(self, exchange: EXCHANGE_FIXTURE_TYPE):
        exch, exchangename = exchange
        pair = EXCHANGES[exchangename]['pair']
        ...
        for val in [1, 2, 5, 25, 100]:
            ...
            if not l2_limit_range or val in l2_limit_range:
                ...
                next_limit = exch.get_next_limit_in_list(val, l2_limit_range)
                if next_limit > 200:
                    # Large orderbook sizes can be a problem for some exchanges (bitrex ...)
                    assert len(l2['asks']) > 200
                    assert len(l2['asks']) > 200
    ...
    @pytest.mark.asyncio
    async def test_ccxt_fetch_ohlcv(self, exchange: EXCHANGE_FIXTURE_TYPE):
        exch, exchangename = exchange
        pair = EXCHANGES[exchangename]['pair']
        ...
        ohlcv = await exch.refresh_latest_ohlcv([pair_tf])
    ...
    async def test_ccxt_fetch_ohlcv_startdate(self, exchange: EXCHANGE_FIXTURE_TYPE):
        """
        ...
        ohlcv = await exch.refresh_latest_ohlcv([pair_tf], since_ms=since_ms)
        assert isinstance(ohlcv, dict)
        assert len(ohlcv[pair_tf]) == len(exch.klines(pair_tf))
        # Check if last-timeframe is within the last 2 intervals
        now = datetime.now(timezone.utc) - timedelta(minutes=(timeframe_to_minutes(timeframe) * 2))
    ...
    async def test_ccxt__async_get_candle_history_futures(
    ...
    async def test_ccxt_fetch_funding_rate_history(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
        exchange, exchangename = exchange_futures
    ...
    async def test_ccxt_fetch_mark_price_history(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
        exchange, exchangename = exchange_futures
        pair = EXCHANGES[exchangename].get('futures_pair', EXCHANGES[exchangename]['pair'])
        since = int((datetime.now(timezone.utc) - timedelta(days=5)).timestamp() * 1000)
    ...
    async def test_ccxt__calculate_funding_fees(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
        exchange, exchangename = exchange_futures
        pair = EXCHANGES[exchangename].get('futures_pair', EXCHANGES[exchangename]['pair'])
    ...
    async def test_ccxt_load_leverage_tiers(self, exchange_futures: EXCHANGE_FIXTURE_TYPE):
        futures, futures_name = exchange_futures
