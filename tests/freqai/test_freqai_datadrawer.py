...
async def test_get_timerange_from_live_historic_predictions(mocker, freqai_conf):
    strategy = await get_patched_freqai_strategy(mocker, freqai_conf)
    exchange = await get_patched_exchange(mocker, freqai_conf)
    strategy.dp = DataProvider(freqai_conf, exchange)
    freqai = strategy.freqai
...
async def test_get_timerange_from_backtesting_live_df_pred_not_found(mocker, freqai_conf):
    strategy = await get_patched_freqai_strategy(mocker, freqai_conf)
    exchange = await get_patched_exchange(mocker, freqai_conf)
    strategy.dp = DataProvider(freqai_conf, exchange)
    freqai = strategy.freqai
