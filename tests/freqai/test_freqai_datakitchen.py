...
async def test_compute_inlier_metric(mocker, freqai_conf, caplog):
    freqai = await make_data_dictionary(mocker, freqai_conf)
...
async def test_add_noise_to_training_features(mocker, freqai_conf):
    freqai = await make_data_dictionary(mocker, freqai_conf)
    freqai_conf['freqai']['feature_parameters'].update({"noise_standard_deviation": 0.1})
    freqai.dk.add_noise_to_training_features()


async def test_remove_beginning_points_from_data_dict(mocker, freqai_conf):
    freqai = await make_data_dictionary(mocker, freqai_conf)
    freqai.dk.remove_beginning_points_from_data_dict(set_='train')


async def test_principal_component_analysis(mocker, freqai_conf, caplog):
    freqai = await make_data_dictionary(mocker, freqai_conf)
...
async def test_normalize_data(mocker, freqai_conf):
    freqai = await make_data_dictionary(mocker, freqai_conf)
...
async def test_filter_features(mocker, freqai_conf):
...
async def test_make_train_test_datasets(mocker, freqai_conf):
    freqai, unfiltered_dataframe = await make_unfiltered_dataframe(mocker, freqai_conf)
    freqai.dk.find_features(unfiltered_dataframe)
...
async def test_get_full_model_path(mocker, freqai_conf, model):
