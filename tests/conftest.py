...
def patch_exchange(
    ...
    # mocker.patch(f'{EXMS}.._load_async_markets', MagicMock(return_value={}))
...
async def get_patched_exchange(
...
def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        ...
        get_mock_coro(return_value=conf["exchange"]["pair_whitelist"]),
    )
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("freqtrade.edge.Edge.calculate", get_mock_coro(return_value=True))
...
async def get_patched_worker(mocker, config) -> Worker:
    ...
    patch_freqtradebot(mocker, config)
    mocker.patch(
        "freqtrade.configuration.configuration.Configuration.get_config", return_value=config
    )
