...
async def test_additional_exchange_init_bybit(default_conf, mocker, caplog):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, exchange="bybit", api_mock=api_mock)
    ...
    with pytest.raises(OperationalException, match=r"Bybit: Unified account is not supported.*"):
        await get_patched_exchange(mocker, default_conf, exchange="bybit", api_mock=api_mock)
...
async def test_bybit_get_funding_fees(default_conf, mocker):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, exchange="bybit")
...
async def test_bybit_fetch_orders(default_conf, mocker, limit_order):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange="bybit")
...
async def test_bybit_fetch_order_canceled_empty(default_conf_usdt, mocker):
    ...
    mocker.patch(f"{EXMS}.exchange_has", return_value=True)
    exchange = await get_patched_exchange(mocker, default_conf_usdt, api_mock, exchange="bybit")

    res = await exchange.fetch_order("123", "BTC/USDT")
    ...
    res1 = await exchange.fetch_order("123", "BTC/USDT")
    ...
    res2 = await exchange.fetch_order("123", "BTC/USDT")
