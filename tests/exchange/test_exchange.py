...
async def test_price_to_precision(default_conf, mocker, price, precision_mode, precision, expected):
...
async def test_get_or_calculate_liquidation_price(mocker, default_conf):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock)
    liq_price = await exchange.get_or_calculate_liquidation_price(
        pair='NEAR/USDT:USDT',
        open_rate=18.884,
        is_short=False,
        position=0.8,
