...
async def ccxt_exceptionhandlers(
    mocker,
    default_conf,
    api_mock,
    exchange_name,
...
async def test_init_ccxt_kwargs(default_conf, mocker, caplog):
    ...
    ex = Exchange(conf)
    await ex.init_exchange(load_markets=False, validate=False)
    assert log_has(
    ...
    assert log_has("Applying additional ccxt config: {'TestKWARG': 11, 'TestKWARG44': 11}", caplog)
    assert ex._api_async.hello == "world"
...
async def test_init_exception(default_conf, mocker):
...
async def test_price_get_one_pip(default_conf, mocker, price, precision_mode, precision, expected):
    markets = PropertyMock(return_value={"ETH/BTC": {"precision": {"price": precision}}})
    exchange = await get_patched_exchange(mocker, default_conf, exchange="binance")
...
async def test__get_stake_amount_limit(mocker, default_conf) -> None:
    exchange = await get_patched_exchange(mocker, default_conf, exchange="binance")
    stoploss = -0.05
...
async def test_get_min_pair_stake_amount_real_data(mocker, default_conf) -> None:
    exchange = await get_patched_exchange(mocker, default_conf, exchange="binance")
    stoploss = -0.05
    markets = {"ETH/BTC": {"symbol": "ETH/BTC"}}
...
async def test_reload_markets(default_conf, mocker, caplog, time_machine):
    caplog.set_level(logging.DEBUG)
...
def test_validate_stakecurrency(default_conf, stake_currency, mocker, caplog):
    # TODO: Evaluate purpose of this test
...
async def test_validate_stakecurrency_error(default_conf, mocker):
...
async def test_get_quote_currencies(default_conf, mocker):
    ex = await get_patched_exchange(mocker, default_conf)
...
async def test_get_pair_quote_currency(default_conf, mocker, pair, expected):
    ex = await get_patched_exchange(mocker, default_conf)
...
async def test_get_pair_base_currency(default_conf, mocker, pair, expected):
    ex = await get_patched_exchange(mocker, default_conf)
...
async def test_validate_pairs_not_available(default_conf, mocker):
    api_mock = MagicMock()
    type(api_mock).load_markets = get_mock_coro(
...
async def test_validate_pairs_exception(default_conf, mocker, caplog):
    caplog.set_level(logging.INFO)
    api_mock = MagicMock()
...
async def test_validate_pairs_restricted(default_conf, mocker, caplog):
    api_mock = MagicMock()
...
async def test_validate_pairs_stakecompatibility_downloaddata(default_conf, mocker):
    api_mock = MagicMock()
...
async def test_validate_pairs_stakecompatibility_fail(default_conf, mocker):
...
def test_validate_timeframes(default_conf, mocker, timeframe):
    # TODO: Evaluate purpose of this test (should it assert something??)
...
async def test_validate_timeframes_failed(default_conf, mocker):
...
async def test_validate_timeframes_emulated_ohlcvi_2(default_conf, mocker):
    default_conf["timeframe"] = "3m"
    api_mock = MagicMock()
    id_mock = PropertyMock(return_value="test_exchange")
    type(api_mock).id = id_mock
...
async def test_validate_pricing(default_conf, mocker):
    api_mock = MagicMock()
    has = {
    ...
    default_conf["exchange"]["name"] = "binance"
    await ExchangeResolver.load_exchange(default_conf)
...
async def test_validate_ordertypes(default_conf, mocker):
    api_mock = MagicMock()
...
def test_validate_order_types_not_in_config(default_conf, mocker):
    ...
    conf = copy.deepcopy(default_conf)
    Exchange(conf)
    # TODO: can this test fail??


async def test_validate_required_startup_candles(default_conf, mocker, caplog):
    api_mock = MagicMock()
    mocker.patch(f"{EXMS}.name", PropertyMock(return_value="Binance"))
...
async def test_exchange_has(default_conf, mocker):
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_create_dry_run_order_limit_fill(
    default_conf,
    mocker,
    side,
    price,
    ...
    mocker.patch(
        f"{EXMS}.fetch_l2_order_book", get_mock_coro(return_value={"asks": [], "bids": []})
    )
...
@pytest.mark.parametrize("leverage", [1, 2, 5])
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_create_dry_run_order_market_fill(
...
async def test_create_order(
    default_conf, mocker, side, ordertype, rate, marketprice, exchange_name
):
    api_mock = MagicMock()
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_buy_dry_run(default_conf, mocker, exchange_name):
    default_conf["dry_run"] = True
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
...
async def test_sell_dry_run(default_conf, mocker):
    default_conf["dry_run"] = True
    exchange = await get_patched_exchange(mocker, default_conf)
...
async def test_sell_prod(default_conf, mocker, exchange_name):
    ...
    with pytest.raises(InvalidOrderException):
        api_mock.create_order = get_mock_coro(side_effect=ccxt.InvalidOrder("Order not found"))
        exchange = await get_patched_exchange(
            mocker, default_conf, api_mock, exchange=exchange_name
        )
        await exchange.create_order(
    ...
    with pytest.raises(TemporaryError):
        api_mock.create_order = get_mock_coro(side_effect=ccxt.NetworkError("No Connection"))
        exchange = await get_patched_exchange(
            mocker, default_conf, api_mock, exchange=exchange_name
        )
        await exchange.create_order(
    ...
    with pytest.raises(OperationalException):
        api_mock.create_order = get_mock_coro(side_effect=ccxt.BaseError("DeadBeef"))
        exchange = await get_patched_exchange(
            mocker, default_conf, api_mock, exchange=exchange_name
        )
        await exchange.create_order(
...
async def test_fetch_positions(default_conf, mocker, exchange_name):
    ...
    api_mock = MagicMock()
    api_mock.fetch_positions = get_mock_coro(
        return_value=[
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_fetch_orders(default_conf, mocker, exchange_name, limit_order):
    api_mock = MagicMock()
    api_mock.fetch_orders = get_mock_coro(
        return_value=[
            limit_order["buy"],
            limit_order["sell"],
        ]
    )
    api_mock.fetch_open_orders = get_mock_coro(return_value=[limit_order["buy"]])
    api_mock.fetch_closed_orders = get_mock_coro(return_value=[limit_order["buy"]])
    ...
    assert await exchange.fetch_orders("mocked", start_time) == []
    assert api_mock.fetch_orders.call_count == 0
    default_conf["dry_run"] = False

    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    res = await exchange.fetch_orders("mocked", start_time)
    ...
    await exchange.fetch_orders("mocked", start_time)
    assert api_mock.fetch_orders.call_count == 0
...
async def test_fetch_trading_fees(default_conf, mocker):
    ...
    mocker.patch(f"{EXMS}.fill_leverage_tiers")
    exchange = await get_patched_exchange(
        mocker, default_conf, api_mock, exchange=exchange_name, mock_markets=False
    )
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_tickers(default_conf, mocker, exchange_name, caplog):
    api_mock = MagicMock()
    tick = {
        "ETH/BTC": {
            "symbol": "ETH/BTC",
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_fetch_ticker_sync(default_conf, mocker, exchange_name):
    api_mock = MagicMock()
    tick = {
        "symbol": "ETH/BTC",
        "bid": 0.00001098,
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    # retrieve original ticker
    ticker = exchange.fetch_ticker_sync(pair="ETH/BTC")
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    exchange.fetch_ticker_sync(pair="ETH/BTC")

    with pytest.raises(DependencyException, match=r"Pair XRP/ETH not available"):
        exchange.fetch_ticker_sync(pair="XRP/ETH")
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_historic_ohlcv(default_conf, mocker, caplog, exchange_name, candle_type):
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    ohlcv = [
        [
...
async def test_refresh_latest_ohlcv(mocker, default_conf, caplog, candle_type) -> None:
    ...
    res = await exchange.refresh_latest_ohlcv(pairlist, cache=False)
    assert len(res) == 3
    ...
    res = await exchange.refresh_latest_ohlcv([("IOTA/ETH", "3m", candle_type)], cache=False)
...
@pytest.mark.parametrize("candle_type", [CandleType.FUTURES, CandleType.MARK, CandleType.SPOT])
async def test_refresh_latest_ohlcv_cache(mocker, default_conf, candle_type, time_machine) -> None:
...
async def test__async_kucoin_get_candle_history(default_conf, mocker, caplog):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange="kucoin")
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_fetch_l2_order_book_sync(default_conf, mocker, exchange_name, order_book_l2_sync):
    default_conf["exchange"]["name"] = exchange_name
    api_mock = MagicMock()

    api_mock.fetch_l2_order_book = order_book_l2_sync
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    order_book = exchange.fetch_l2_order_book_sync(pair="ETH/BTC", limit=10)
    ...
    for val in [1, 5, 10, 12, 20, 50, 100]:
        ...
        order_book = exchange.fetch_l2_order_book_sync(pair="ETH/BTC", limit=val)
        assert api_mock.fetch_l2_order_book.call_args_list[0][0][0] == "ETH/BTC"
    ...
    with pytest.raises(OperationalException):
        ...
        exchange.fetch_l2_order_book_sync(pair="ETH/BTC", limit=50)
    with pytest.raises(TemporaryError):
...
async def test_get_entry_rate(
    ...
    assert (
        await exchange.get_rate("ETH/BTC", side="entry", is_short=False, refresh=True) == expected
    )
    ...
    caplog.clear()
    assert (
        await exchange.get_rate("ETH/BTC", side="entry", is_short=False, refresh=False) == expected
    )
...
async def test_get_ticker_rate_error(
    ...
    with pytest.raises(PricingError):
        await exchange.get_rate("ETH/BTC", refresh=True, side=entry, is_short=is_short)
...
async def test_get_exit_rate_orderbook(
    ...
    exchange = await get_patched_exchange(mocker, default_conf)
    rate = await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short)
    assert not log_has("Using cached exit rate for ETH/BTC.", caplog)
    assert isinstance(rate, float)
    assert rate == expected
    rate = await exchange.get_rate(pair, refresh=False, side="exit", is_short=is_short)
    assert rate == expected
    assert log_has("Using cached exit rate for ETH/BTC.", caplog)


async def test_get_exit_rate_orderbook_exception(default_conf, mocker, caplog):
    # Test orderbook mode
    ...
    exchange = await get_patched_exchange(mocker, default_conf)
    with pytest.raises(PricingError):
        await exchange.get_rate(pair, refresh=True, side="exit", is_short=False)
    assert log_has_re(
...
async def test_get_exit_rate_exception(default_conf, mocker, is_short):
    # Ticker on one side can be empty in certain circumstances.
    default_conf["exit_pricing"]["price_side"] = "ask"
    pair = "ETH/BTC"
    mocker.patch(f"{EXMS}.fetch_ticker", get_mock_coro({"ask": None, "bid": 0.12, "last": None}))
    exchange = await get_patched_exchange(mocker, default_conf)
    with pytest.raises(PricingError, match=r"Exit-Rate for ETH/BTC was empty."):
        await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short)

    exchange._config["exit_pricing"]["price_side"] = "bid"
    assert await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short) == 0.12
    # Reverse sides
    mocker.patch(f"{EXMS}.fetch_ticker", get_mock_coro({"ask": 0.13, "bid": None, "last": None}))
    with pytest.raises(PricingError, match=r"Exit-Rate for ETH/BTC was empty."):
        await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short)

    exchange._config["exit_pricing"]["price_side"] = "ask"
    assert await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short) == 0.13


@pytest.mark.parametrize("side,ask,bid,last,last_ab,expected", get_entry_rate_data)
@pytest.mark.parametrize("side2", ["bid", "ask"])
@pytest.mark.parametrize("use_order_book", [True, False])
async def test_get_rates_testing_entry(
    mocker,
    default_conf,
    caplog,
    side,
    ...
    api_mock = MagicMock()
    api_mock.fetch_l2_order_book = order_book_l2
    api_mock.fetch_ticker = get_mock_coro(return_value={"ask": ask, "last": last, "bid": bid})
    exchange = await get_patched_exchange(mocker, default_conf, api_mock)

    assert (await exchange.get_rates("ETH/BTC", refresh=True, is_short=False))[0] == expected
    assert not log_has("Using cached buy rate for ETH/BTC.", caplog)

    api_mock.fetch_l2_order_book.reset_mock()
    api_mock.fetch_ticker.reset_mock()
    assert (await exchange.get_rates("ETH/BTC", refresh=False, is_short=False))[0] == expected
...
@pytest.mark.parametrize("side2", ["bid", "ask"])
@pytest.mark.parametrize("use_order_book", [True, False])
async def test_get_rates_testing_exit(
    default_conf,
    mocker,
    caplog,
    side,
    ...
    # Test regular mode
    rate = (await exchange.get_rates(pair, refresh=True, is_short=False))[1]
    ...
    rate = (await exchange.get_rates(pair, refresh=False, is_short=False))[1]
    assert rate == expected
...
async def test__valid_trade_pagination_id(
    ...
    exchange = await get_patched_exchange(mocker, default_conf_usdt, exchange=exchange_name)
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_historic_trades(default_conf, mocker, caplog, exchange_name, trades_history):
    mocker.patch(f"{EXMS}.exchange_has", return_value=True)
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    ...
    ret = await exchange.get_historic_trades(
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_historic_trades_notsupported(
    default_conf, mocker, caplog, exchange_name, trades_history
):
    mocker.patch(f"{EXMS}.exchange_has", return_value=False)
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    ...
    with pytest.raises(
        ...
        await exchange.get_historic_trades(
            pair, since=trades_history[0][0], until=trades_history[-1][0]
        )
...
async def test_cancel_order_dry_run(default_conf, mocker, exchange_name):
    default_conf["dry_run"] = True
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    mocker.patch(f"{EXMS}._dry_is_price_crossed", get_mock_coro(return_value=True))
...
async def test_check_order_canceled_empty(mocker, default_conf, exchange_name, order, result):
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
...
async def test_cancel_order_with_result(
...
async def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    ...
    mocker.patch(f"{EXMS}.fetch_stoploss_order", get_mock_coro({"for": 123}))
    mocker.patch(f"{mock_prefix}.fetch_stoploss_order", get_mock_coro({"for": 123}))
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)

    res = {"fee": {}, "status": "canceled", "amount": 1234}
    mocker.patch(f"{EXMS}.cancel_stoploss_order", get_mock_coro(res))
    mocker.patch(f"{mock_prefix}.cancel_stoploss_order", get_mock_coro(res))
    co = await exchange.cancel_stoploss_order_with_result(order_id="_", pair="TKN/BTC", amount=555)
    assert co == res

    mocker.patch(f"{EXMS}.cancel_stoploss_order", get_mock_coro("canceled"))
    mocker.patch(f"{mock_prefix}.cancel_stoploss_order", get_mock_coro("canceled"))
...
async def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    assert await exchange.fetch_order("X", "TKN/BTC") == {
        "id": "123",
        "amount": 2,
        "symbol": "TKN/BTC",
    }
...
@pytest.mark.usefixtures("init_persistence")
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_fetch_order_emulated(default_conf, mocker, exchange_name, caplog):
    default_conf["dry_run"] = True
    default_conf["exchange"]["log_responses"] = True
    order = MagicMock()
    order.myid = 123
    ...
    mocker.patch(f"{EXMS}.exchange_has", return_value=False)
    api_mock = MagicMock()
    api_mock.fetch_open_order = get_mock_coro(
        return_value={"id": "123", "amount": 2, "symbol": "TKN/BTC"}
    )
    api_mock.fetch_closed_order = get_mock_coro(
        return_value={"id": "123", "amount": 2, "symbol": "TKN/BTC"}
    )
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    assert await exchange.fetch_order("X", "TKN/BTC") == {
    ...
    api_mock.fetch_open_order = get_mock_coro(side_effect=ccxt.OrderNotFound("Order not found"))
    api_mock.fetch_closed_order = get_mock_coro(
        return_value={"id": "123", "amount": 2, "symbol": "TKN/BTC"}
    )
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    ...
    with pytest.raises(InvalidOrderException):
        api_mock.fetch_open_order = get_mock_coro(side_effect=ccxt.InvalidOrder("Order not found"))
        api_mock.fetch_closed_order = get_mock_coro(
            side_effect=ccxt.InvalidOrder("Order not found")
        )
        exchange = await get_patched_exchange(
            mocker, default_conf, api_mock, exchange=exchange_name
        )
...
async def test_fetch_stoploss_order(default_conf, mocker, exchange_name):
    ...
    default_conf["dry_run"] = False
    api_mock = MagicMock()
    api_mock.fetch_order = get_mock_coro(return_value={"id": "123", "symbol": "TKN/BTC"})
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
    res = {"id": "123", "symbol": "TKN/BTC"}
    if exchange_name == "okx":
        res = {"id": "123", "symbol": "TKN/BTC", "type": "stoploss"}
    assert await exchange.fetch_stoploss_order("X", "TKN/BTC") == res
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_trades_for_order(default_conf, mocker, exchange_name, trading_mode, amount):
    order_id = "ABCD-ABCD"
    ...
    api_mock.fetch_my_trades = get_mock_coro(
        return_value=[
            {
    ...
    orders = await exchange.get_trades_for_order(order_id, "ETH/USDT:USDT", since)
    assert len(orders) == 1
    ...
    assert await exchange.get_trades_for_order(order_id, "ETH/USDT:USDT", since) == []


@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_fee(default_conf, mocker, exchange_name):
    api_mock = MagicMock()
...
async def test_stoploss_order_unsupported_exchange(default_conf, mocker):
    exchange = await get_patched_exchange(mocker, default_conf, exchange="bitpanda")
...
async def test__get_stop_limit_rate(default_conf_usdt, mocker, side, ratio, expected):
    exchange = await get_patched_exchange(mocker, default_conf_usdt, exchange="binance")
...
async def test_get_markets_error(default_conf, mocker):
    ex = await get_patched_exchange(mocker, default_conf)
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_ohlcv_candle_limit(default_conf, mocker, exchange_name):
    if exchange_name == "okx":
...
async def test_market_is_tradable(
    mocker,
    default_conf,
    market_symbol,
    base,
    ...
    mocker.patch(f"{EXMS}.validate_trading_mode_and_margin_mode")
    ex = await get_patched_exchange(mocker, default_conf, exchange=exchange)
    market = {
...
async def test_extract_cost_curr_rate(mocker, default_conf, order, expected) -> None:
    mocker.patch(f"{EXMS}.calculate_fee_rate", get_mock_coro(return_value=0.01))
    ex = await get_patched_exchange(mocker, default_conf)
    assert (
        await ex.extract_cost_curr_rate(order["fee"], order["symbol"], cost=20, amount=1)
    ) == expected
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_get_funding_fees(default_conf_usdt, mocker, exchange_name, caplog):
    now = datetime.now(timezone.utc)
    default_conf_usdt["trading_mode"] = "futures"
    default_conf_usdt["margin_mode"] = "isolated"
    exchange = await get_patched_exchange(mocker, default_conf_usdt, exchange=exchange_name)
    exchange._fetch_and_calculate_funding_fees = get_mock_coro(side_effect=ExchangeError())
    assert await exchange.get_funding_fees("BTC/USDT:USDT", 1, False, now) == 0.0
...
async def test__get_funding_fees_from_exchange(default_conf, mocker, exchange_name):
    api_mock = MagicMock()
    api_mock.fetch_funding_history = get_mock_coro(
        return_value=[
            {
                "amount": 0.14542,
                "code": "USDT",
...
async def test_get_stake_amount_considering_leverage(
    ...
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange)
    assert (
...
async def test__ccxt_config(default_conf, mocker, exchange_name, trading_mode, ccxt_config):
    default_conf["trading_mode"] = trading_mode
    default_conf["margin_mode"] = "isolated"
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
...
async def test_get_max_leverage_from_margin(default_conf, mocker, pair, nominal_value, max_lev):
    default_conf["trading_mode"] = "margin"
    default_conf["margin_mode"] = "isolated"
    api_mock = MagicMock()
    type(api_mock).has = PropertyMock(return_value={"fetchLeverageTiers": False})
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange="gate")
...
async def test_calculate_funding_fees(
    default_conf, mocker, size, funding_rate, mark_price, funding_fee, kraken_fee, time_in_ratio
):
    exchange = await get_patched_exchange(mocker, default_conf)
    kraken = await get_patched_exchange(mocker, default_conf, exchange="kraken")
...
async def test_combine_funding_and_mark(
    default_conf,
    mocker,
    funding_rate,
    mark_price,
...
async def test__fetch_and_calculate_funding_fees(
    mocker,
    default_conf,
    ...
    ex = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange)
...
async def test__fetch_and_calculate_funding_fees_datetime_called(
    mocker,
    default_conf,
    funding_rate_history_octohourly,
    mark_ohlcv,
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange)
...
async def test__get_contract_size(mocker, default_conf, pair, expected_size, trading_mode):
    api_mock = MagicMock()
    default_conf["trading_mode"] = trading_mode
    default_conf["margin_mode"] = "isolated"
    exchange = await get_patched_exchange(mocker, default_conf, api_mock)
...
async def test__order_contracts_to_amount(
    mocker,
    default_conf,
    markets,
    pair,
...
async def test__trades_contracts_to_amount(
    mocker,
    default_conf,
    markets,
    pair,
...
async def test__amount_to_contracts(mocker, default_conf, pair, param_amount, param_size):
    api_mock = MagicMock()
...
async def test_amount_to_contract_precision(
    mocker,
    default_conf,
    pair,
    amount,
...
async def test_liquidation_price_is_none(
    mocker,
    default_conf,
    exchange_name,
    open_rate,
    ...
    exchange = await get_patched_exchange(mocker, default_conf, exchange=exchange_name)
    assert (
        await exchange.get_liquidation_price(
            pair="DOGE/USDT",
            open_rate=open_rate,
            is_short=is_short,
            amount=71200.81144,
...
async def test_liquidation_price_binance(
    mocker,
    default_conf,
    exchange_name,
    open_rate,
...
async def test_get_max_pair_stake_amount(
    mocker,
    default_conf,
):
    api_mock = MagicMock()
...
@pytest.mark.parametrize("exchange_name", EXCHANGES)
async def test_load_leverage_tiers(mocker, default_conf, exchange_name):
    if exchange_name == "bybit":
    ...
    api_mock.fetch_leverage_tiers = get_mock_coro(
        return_value={
            "ADA/USDT:USDT": [
                {
                    "tier": 1,
...
async def test_parse_leverage_tier(mocker, default_conf):
    exchange = await get_patched_exchange(mocker, default_conf)
...
async def test_get_maintenance_ratio_and_amt_exceptions(mocker, default_conf, leverage_tiers):
    api_mock = MagicMock()
    default_conf["trading_mode"] = "futures"
    default_conf["margin_mode"] = "isolated"
    mocker.patch(f"{EXMS}.exchange_has", return_value=True)
...
async def test_get_maintenance_ratio_and_amt(
...
async def test_get_max_leverage_futures(default_conf, mocker, leverage_tiers):
    # Test Spot
    exchange = await get_patched_exchange(mocker, default_conf, exchange="binance")
...
async def test__get_params(mocker, default_conf, exchange_name):
    api_mock = MagicMock()
    mocker.patch(f"{EXMS}.exchange_has", return_value=True)
    mocker.patch(f"{EXMS}.get_leverage_tiers", get_mock_coro({}))
    exchange = await get_patched_exchange(mocker, default_conf, api_mock, exchange=exchange_name)
...
async def test_get_liquidation_price1(mocker, default_conf):
    ...
    api_mock.fetch_positions = get_mock_coro(return_value=positions)
    mocker.patch.multiple(
        EXMS,
        exchange_has=MagicMock(return_value=True),
    )
    ...
    exchange = await get_patched_exchange(mocker, default_conf, api_mock)
    liq_price = await exchange.get_liquidation_price(
        pair="NEAR/USDT:USDT",
        open_rate=18.884,
        is_short=False,
        amount=0.8,
...
async def test_get_liquidation_price(
    mocker,
    default_conf_usdt,
    is_short,
    trading_mode,
