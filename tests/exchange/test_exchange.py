...
async def test_create_dry_run_order_limit_fill(
        default_conf, mocker, side, price, filled, caplog, exchange_name, order_book_l2_usd,
        converted, leverage):
    default_conf['dry_run'] = True
    exchange = await get_patched_exchange(mocker, default_conf, id=exchange_name)
    ...
    mocker.patch(f'{EXMS}.fetch_l2_order_book',
                 get_mock_coro(return_value={'asks': [], 'bids': []}))
...
async def test_buy_dry_run(default_conf, mocker, exchange_name):
    ...
    order = await exchange.create_order(pair='ETH/BTC', ordertype='limit', side="buy",
...
async def test_buy_prod(default_conf, mocker, exchange_name):
    ...
    with pytest.raises(DependencyException):
        ...
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype=order_type, side="buy",
                                    amount=1, rate=200, leverage=1.0,
                                    time_in_force=time_in_force)
    ...
    with pytest.raises(DependencyException):
        ...
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype='limit', side="buy",
                                    amount=1, rate=200, leverage=1.0,
                                    time_in_force=time_in_force)
    ...
    with pytest.raises(DependencyException):
        api_mock.create_order = MagicMock(side_effect=ccxt.InvalidOrder("Order not found"))
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype='market', side="buy",
                                    amount=1, rate=200, leverage=1.0,
                                    time_in_force=time_in_force)
...
async def test_sell_dry_run(default_conf, mocker):
    ...
    order = await exchange.create_order(pair='ETH/BTC', ordertype='limit',
...
async def test_sell_prod(default_conf, mocker, exchange_name):
    ...
    with pytest.raises(InsufficientFundsError):
        ...
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype=order_type, side="sell", amount=1,
                                    rate=200, leverage=1.0)

    with pytest.raises(InvalidOrderException):
        api_mock.create_order = get_mock_coro(side_effect=ccxt.InvalidOrder("Order not found"))
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype='limit', side="sell", amount=1,
                                    rate=200, leverage=1.0)
    ...
    with pytest.raises(DependencyException):
        api_mock.create_order = get_mock_coro(side_effect=ccxt.InvalidOrder("Order not found"))
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.create_order(pair='ETH/BTC', ordertype='market', side="sell", amount=1,
                                    rate=200, leverage=1.0)
...
async def test_sell_considers_time_in_force(default_conf, mocker, exchange_name):
    ...
    order = await exchange.create_order(pair='ETH/BTC', ordertype=order_type, side="sell",
                                        amount=1, rate=200, leverage=1.0,
                                        time_in_force=time_in_force)
...
async def test_get_ticker_rate_error(mocker, entry, default_conf, caplog, side, is_short, ask, bid,
...
async def test_get_exit_rate_exception(default_conf, mocker, is_short):
    ...
    pair = "ETH/BTC"
    mocker.patch(f'{EXMS}.fetch_ticker',
                 get_mock_coro({'ask': None, 'bid': 0.12, 'last': None}))
    exchange = await get_patched_exchange(mocker, default_conf)
    ...
    # Reverse sides
    mocker.patch(f'{EXMS}.fetch_ticker',
                 get_mock_coro({'ask': 0.13, 'bid': None, 'last': None}))
    with pytest.raises(PricingError, match=r"Exit-Rate for ETH/BTC was empty."):
        await exchange.get_rate(pair, refresh=True, side="exit", is_short=is_short)
...
async def test_get_rates_testing_entry(mocker, default_conf, caplog, side, ask, bid,
    ...
    api_mock = MagicMock()
    api_mock.fetch_l2_order_book = order_book_l2
    api_mock.fetch_ticker = get_mock_coro(
        return_value={'ask': ask, 'last': last, 'bid': bid})
    exchange = await get_patched_exchange(mocker, default_conf, api_mock)
...
async def test_cancel_order_dry_run(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = True
    exchange = await get_patched_exchange(mocker, default_conf, id=exchange_name)
    mocker.patch(f'{EXMS}._dry_is_price_crossed',
...
async def test_fetch_order_emulated(default_conf, mocker, exchange_name, caplog):
    ...
    with pytest.raises(InvalidOrderException):
        ...
        api_mock.fetch_closed_order = get_mock_coro(side_effect=ccxt.InvalidOrder("Order not found"))
        exchange = await get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
        await exchange.fetch_order(order_id='_', pair='TKN/BTC')
    assert api_mock.fetch_open_order.call_count == 1
...
async def test__amount_to_contracts(
    mocker,
    default_conf,
    pair,
    param_amount,
...
async def test_liquidation_price_is_none(
    ...
    exchange = await get_patched_exchange(mocker, default_conf, id=exchange_name)
    assert await exchange.get_liquidation_price(
        pair='DOGE/USDT',
        open_rate=open_rate,
        is_short=is_short,
        amount=71200.81144,
...
async def test_liquidation_price_binance(
    ...
    assert pytest.approx(round(await exchange.get_liquidation_price(
        pair='DOGE/USDT',
        open_rate=open_rate,
        is_short=is_short,
