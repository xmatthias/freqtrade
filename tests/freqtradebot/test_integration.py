...
async def test_may_execute_exit_stoploss_on_exchange_multi(
    default_conf, ticker, fee, mocker
) -> None:
    """
    ...
    * 3rd trade is sold via sell-signal
    ...
    stoploss_order_mock = get_mock_coro(side_effect=stop_orders)
...
@pytest.mark.asyncio
async def test_forcebuy_last_unlimited(
    default_conf, ticker, fee, mocker, balance_ratio, result1
) -> None:
    """
...
async def test_dca_buying(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    ...
    mocker.patch(f"{EXMS}.fetch_ticker", get_mock_coro(ticker_usdt_modif))
...
async def test_dca_short(default_conf_usdt, ticker_usdt, fee, mocker) -> None:
    default_conf_usdt["position_adjustment_enable"] = True
...
async def test_dca_order_adjust(default_conf_usdt, ticker_usdt, leverage, fee, mocker) -> None:
    default_conf_usdt["position_adjustment_enable"] = True
    ...
    patch_get_signal(freqtrade)
    new_val = (await ticker_usdt())["ask"] * 0.96
    freqtrade.strategy.custom_entry_price = lambda **kwargs: new_val
...
async def test_dca_order_adjust_entry_replace_fails(
...
@pytest.mark.parametrize("leverage", [1, 2])
async def test_dca_exiting(default_conf_usdt, ticker_usdt, fee, mocker, caplog, leverage) -> None:
    default_conf_usdt["position_adjustment_enable"] = True
    spot = leverage == 1
    if not spot:
        default_conf_usdt["trading_mode"] = "futures"
