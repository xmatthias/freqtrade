...
async def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert (
        await freqtrade.wallets.get_trade_stake_amount("NEO/BTC", 1, freqtrade.edge)
        == (999.9 * 0.5 * 0.01) / 0.20
    )
    assert (
        await freqtrade.wallets.get_trade_stake_amount("LTC/BTC", 1, freqtrade.edge)
        == (999.9 * 0.5 * 0.01) / 0.21
    )
...
async def test_edge_overrides_stoploss(
...
async def test_create_trade_minimal_amount(
    ...
    if create:
        ...
        if not max_open_trades:
            assert (
                await freqtrade.wallets.get_trade_stake_amount(
...
async def test_create_trades_preopen(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_process_trade_creation(
...
async def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch(
        "freqtrade.configuration.configuration.Configuration.get_config",
...
async def test_process_trade_no_whitelist_pair(
...
async def test_execute_entry_confirm_error(
    mocker, default_conf_usdt, fee, limit_order, is_short
) -> None:
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mocker.patch.multiple(
        EXMS,
...
async def test_enter_positions(
    ...
    mock_ct = get_mock_coro(
        ...
        side_effect=[
            DependencyException()
            for _ in range(len(default_conf_usdt["exchange"]["pair_whitelist"]))
        ]
        if side_effect
        else None,
    )
    mocker.patch("freqtrade.freqtradebot.FreqtradeBot.create_trade", mock_ct)
    n = await freqtrade.enter_positions()
    assert n == 0
...
async def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch(f"{EXMS}.fetch_order", get_mock_coro(limit_order[entry_side(is_short)]))
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_update_trade_state_exception(
...
async def test_handle_trade(
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=get_mock_coro(return_value={"bid": 2.19, "ask": 2.2, "last": 2.19}),
        create_order=get_mock_coro(
            side_effect=[
                enter_order,
                open_order,
            ]
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_handle_trade_roi(
...
async def test_manage_open_orders_exception(
...
async def test_handle_cancel_enter(
    mocker, caplog, default_conf_usdt, limit_order, is_short, fee
) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
...
async def test_handle_cancel_enter_exchanges(
...
@pytest.mark.usefixtures("init_persistence")
async def test_handle_cancel_enter_corder_empty(
...
async def test_handle_cancel_exit_limit(
...
async def test_execute_trade_exit_up(
    ...
    price = (
        (await ticker_usdt_sell_down())["ask"] if is_short else (await ticker_usdt_sell_up())["bid"]
    )
    # Prevented sell ...
    await freqtrade.execute_trade_exit(
        trade=trade, limit=price, exit_check=ExitCheckTuple(exit_type=ExitType.ROI)
    )
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_execute_trade_exit_down(
    default_conf_usdt,
    ticker_usdt,
    fee,
    ticker_usdt_sell_down,
...
async def test_exit_profit_only(
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=get_mock_coro(return_value={"bid": bid, "ask": ask, "last": bid}),
        create_order=get_mock_coro(
            side_effect=[
                limit_order[eside],
                {"id": 1234553382},
...
async def test_sell_not_enough_balance(
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_ignore_roi_if_entry_signal(
...
async def test_trailing_stop_loss(
    ...
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=get_mock_coro(return_value={"bid": 2.0, "ask": 2.0, "last": 2.0}),
        create_order=get_mock_coro(
            side_effect=[
    ...
    mocker.patch(
        f"{EXMS}.fetch_ticker",
        get_mock_coro(return_value={"bid": 2.0 * val1, "ask": 2.0 * val1, "last": 2.0 * val1}),
    )
    ...
    mocker.patch(
        f"{EXMS}.fetch_ticker",
        get_mock_coro(return_value={"bid": 2.0 * val2, "ask": 2.0 * val2, "last": 2.0 * val2}),
    )
...
@pytest.mark.parametrize("is_short", [False, True])
async def test_disable_ignore_roi_if_entry_signal(
    default_conf_usdt, limit_order, limit_order_open, is_short, fee, mocker
) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
...
async def test_get_real_amount_quote_dust(
    default_conf_usdt, trades_for_order, buy_order_fee, fee, caplog, mocker
):
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=trades_for_order)
...
async def test_get_real_amount_multi(
    ...
    freqtrade = await get_patched_freqtradebot(mocker, default_conf_usdt)
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    mocker.patch(f"{EXMS}.fetch_ticker", get_mock_coro(return_value={"ask": 0.19, "last": 0.2}))
...
async def test_get_real_amount_fees_order(
    ...
    assert (
        await freqtrade.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
    )
...
async def test_get_real_amount_wrong_amount(
    default_conf_usdt, trades_for_order, buy_order_fee, fee, mocker
):
    limit_buy_order_usdt = deepcopy(buy_order_fee)
...
async def test_apply_fee_conditional(
    ...
    # Amount is kept as is
    assert (
        await freqtrade.apply_fee_conditional(trade, "LTC", amount, fee_abs, order)
    ) == amount_exp
    assert walletmock.call_count == 1
...
async def test_apply_fee_conditional_multibuy(
    default_conf_usdt, fee, mocker, caplog, amount, fee_abs, wallet, amount_exp
):
    walletmock = mocker.patch("freqtrade.wallets.Wallets.update")
    mocker.patch("freqtrade.wallets.Wallets.get_free", return_value=wallet)
    ...
    assert (
        await freqtrade.apply_fee_conditional(trade, "LTC", amount, fee_abs, order1) == amount_exp
    )
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
async def test_order_book_entry_pricing1(
    mocker,
    default_conf_usdt,
    order_book_l2,
    exception_thrown,
...
async def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert (
        await freqtrade._check_depth_of_market("ETH/BTC", conf, side=SignalDirection.LONG)
    ) is False
...
async def test_order_book_exit_pricing(
    ...
    mocker.patch(
        f"{EXMS}.fetch_l2_order_book", get_mock_coro(return_value={"bids": [[]], "asks": [[]]})
    )
    with pytest.raises(PricingError):
        await freqtrade.handle_trade(trade)
    assert log_has_re(
...
async def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    mock_fo = mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value={"status": "open"})
    )
...
async def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch.multiple(
        ...
        fetch_ticker=get_mock_coro(return_value={"bid": 10, "ask": 12, "last": 11}),
        get_min_pair_stake_amount=MagicMock(return_value=1),
        get_balances=get_mock_coro([]),
        get_fee=fee,
    )
    ...
    mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value=open_dca_order_1)
    )
    ...
    mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value=closed_dca_order_1)
    )
    await freqtrade.manage_open_orders()
    ...
    mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value=closed_dca_order_2)
    )
    assert await freqtrade.execute_entry(pair, stake_amount, trade=trade)
    ...
    mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value=closed_sell_dca_order_1)
    )
...
async def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch(
        f"{EXMS}.fetch_order_or_stoploss_order", get_mock_coro(return_value=closed_sell_dca_order_2)
    )
    assert await freqtrade.execute_trade_exit(
        trade=trade,
        limit=ask,
