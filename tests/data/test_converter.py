...
async def test_convert_trades_format(default_conf, testdatadir, tmp_path):
    files = [
    ...
    await convert_trades_format(default_conf, convert_from="jsongz", convert_to="json", erase=False)
    ...
    for file in files:
        ...
        file["old"].unlink()
    # Convert back
    await convert_trades_format(default_conf, convert_from="json", convert_to="jsongz", erase=True)
    for file in files:
        assert file["old"].exists()
        assert not file["new"].exists()
