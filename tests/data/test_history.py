...
from tests.conftest import (CURRENT_TEST_STRATEGY, EXMS, get_mock_coro, get_patched_exchange,
...
async def test_download_pair_history(
    ...
    assert await _download_pair_history(datadir=tmp_path, exchange=exchange,
                                        pair='MEME/BTC',
                                        timeframe='1m',
...
async def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    await _download_pair_history(datadir=testdatadir, exchange=exchange, pair="UNITTEST/USDT",
...
async def test_download_backtesting_data_exception(mocker, caplog, default_conf, tmp_path) -> None:
    ...
    assert not await _download_pair_history(datadir=tmp_path, exchange=exchange,
                                            pair='MEME/BTC',
                                            timeframe='1m', candle_type='spot')
