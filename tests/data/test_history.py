...
async def test_load_data_with_new_pair_1min(
...
async def test_download_pair_history(
    ...
    assert await _download_pair_history(
        datadir=tmp_path,
        exchange=exchange,
        pair="MEME/BTC",
        timeframe="1m",
...
async def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    tick = [
...
async def test_download_backtesting_data_exception(mocker, caplog, default_conf, tmp_path) -> None:
    mocker.patch(f"{EXMS}.get_historic_ohlcv", side_effect=Exception("File Error"))
    exchange = await get_patched_exchange(mocker, default_conf)

    assert not await _download_pair_history(
...
async def test_refresh_backtest_ohlcv_data(
...
async def test_download_data_no_markets(mocker, default_conf, caplog, testdatadir):
    dl_mock = mocker.patch(
        "freqtrade.data.history.history_utils._download_pair_history", get_mock_coro()
    )

    ex = await get_patched_exchange(mocker, default_conf)
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value={}))
    timerange = TimeRange.parse_timerange("20190101-20190102")
    unav_pairs = await refresh_backtest_ohlcv_data(
        exchange=ex,
...
async def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch(
        "freqtrade.data.history.history_utils._download_trades_history", get_mock_coro()
    )
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    mocker.patch.object(Path, "exists", MagicMock(return_value=True))
    mocker.patch.object(Path, "unlink", MagicMock())
...
async def test_download_trades_history(
    ...
    assert await _download_trades_history(
    ...
    mocker.patch(f"{EXMS}.get_historic_trades", get_mock_coro(side_effect=ValueError))
    caplog.clear()

    assert not await _download_trades_history(
        data_handler=data_handler, exchange=exchange, pair="ETH/BTC", trading_mode=TradingMode.SPOT
    )
